...
def _norm_layer(kind: str, num_channels: int) -> nn.Module:
    """
    Returns a normalization layer based on `kind`.
    - "instance": InstanceNorm2d(affine=True)
    - "none": Identity (i.e., no normalization)
    """
    kind = kind.lower()
    if kind == "instance":
        # Running stats are not used for InstanceNorm by default.
        return nn.InstanceNorm2d(num_channels, affine=True)
    elif kind == "none":
        return nn.Identity()
    else:
        raise ValueError(f"Unsupported norm kind: {kind!r}. Use 'instance' or 'none'.")
